...
void browser.storage.local.get(FONT_LIST_KEY).then(saved => {
  ...
  if (names.length) status.textContent = `已保存 ${names.length} 个字体家族，可点击更新。`;
}).catch(() => { if (version === 0) status.textContent = '无法读取已保存列表，可以重新扫描。'; });
scan.addEventListener('click', async () => {
  version++; scan.disabled = true; clear.disabled = true;
  status.textContent = '正在请求字体权限并读取列表…';
  try {
    const names = await readLocalFonts(window as Window & { queryLocalFonts?: () => Promise<{ family: string }[]> });
    if (!names.length) { status.textContent = '浏览器未返回字体，保留原列表；可以使用系统备用字体。'; return; }
    await browser.storage.local.set({ [FONT_LIST_KEY]: names });
    show(names); status.textContent = `已读取并保存 ${names.length} 个字体家族。请回到 Netflix 扩展弹窗选择字体。`;
  } catch (error) { status.textContent = error instanceof Error ? error.message : '字体列表保存失败，请重试。'; }
  finally { scan.disabled = false; clear.disabled = false; }
});
clear.addEventListener('click', async () => {
  version++; scan.disabled = true; clear.disabled = true;
  try { await browser.storage.local.remove(FONT_LIST_KEY); show([]); status.textContent = '已清除列表，可重新读取。'; }
