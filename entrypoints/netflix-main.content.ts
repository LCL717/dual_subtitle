...
export default defineContentScript({
  ...
  main() {
    ...
    window.addEventListener('message', (event: MessageEvent) => {
      ...
      if (!message || !['dual:tracks-request:v1', 'dual:resources-request:v1', 'dual:load-request:v1', 'dual:cancel-load:v1', 'dual:clock-request:v1'].includes(message.type)
        || typeof message.id !== 'string' || message.id.length > 80) return;
      if (message.type === 'dual:clock-request:v1') {
        ...
        if (message.debug === true) {
          ...
          window.postMessage({ type: 'dual:debug-clock-response:v1', id: message.id, path: location.pathname,
        ...
        window.postMessage({ type: 'dual:clock-response:v1', id: message.id, path: location.pathname, sample }, location.origin);
        return;
      }
      if (message.type === 'dual:cancel-load:v1') { loadController?.abort(); return; }
      if (message.type === 'dual:load-request:v1') {
        ...
        void loadSubtitles((window as unknown as Record<string, unknown>).netflix, message.ids, controller.signal,
          ...
          window.postMessage({ type: 'dual:load-response:v1', id: message.id, payload }, location.origin);
        }).catch(error => {
          window.postMessage({ type: 'dual:load-response:v1', id: message.id,
      ...
      if (message.type === 'dual:resources-request:v1') {
        if (message.path !== location.pathname || !Array.isArray(message.ids) || message.ids.length !== 2
          || !message.ids.every((id: unknown) => typeof id === 'string' && id.length <= 200)) return;
        ...
        void inspectPlayerResources((window as unknown as Record<string, unknown>).netflix, message.ids,
          ...
          window.postMessage({ type: 'dual:resources-response:v1', id: message.id, report }, location.origin);
        }).catch(() => {
          window.postMessage({ type: 'dual:resources-response:v1', id: message.id,
      ...
      window.postMessage({ type: 'dual:tracks-response:v1', id: message.id, report }, location.origin);
    });
  },
});
