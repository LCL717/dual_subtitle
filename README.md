...
已完成项目骨架：扩展弹窗、Netflix 播放器按需检测、两个语言选择框占位、字号预览与本机保存。

**目前不能显示 Netflix 双语字幕。** 语言选择和开启按钮暂时禁用；没有伪造字幕语言列表。下一步是验证当前影片字幕菜单及两条字幕轨道的获取。真实浏览器交互与 Netflix 验收尚未完成。
...
语言列表尚未接入，因此检测到播放器也不代表双语字幕已可用。预览中的文字是固定示例，与当前影片无关。
